...
function createChatPanel(chatId, name) {
    ...
    panel.draggable = true; // 드래그 가능하게 설정
    ...
    panel.innerHTML = `
        ...
        <div class="panel-footer">
            <input type="text" class="panel-input" id="input-${chatId}" placeholder="메세지 입력...">
...
function setupDragAndDrop() {
    ...
    container.addEventListener('dragover', (e) => {
        ...
        const siblings = [...container.querySelectorAll('.chat-panel:not(.dragging)')];
...
async function sendMessage(chatId, text = null) {
    ...
    const msgText = text || (input ? input.value : null);
    if (!msgText) return;
    ...
    try {
        ...
        if (data.actions) {
            data.actions.forEach(action => {
                ...
                appendPlannerMessage('received', `✅ [${action.targetName}] 방에 전달 완료`);
            });
        }
    } catch (e) { appendPlannerMessage('received', '오류 발생'); }
};
